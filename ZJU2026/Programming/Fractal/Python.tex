...
% \end{itemize}
% \end{frame}
\section{用 Shell 脚本生成 Mandelbrot 放大电影}

\begin{frame}{现在考虑一下如何实现这个效果}
\centering
\href{run:mandelbrot_zoom.mp4}{
  \includegraphics[width=0.6\textwidth]{mandelbrot.bmp}
}
...
\begin{frame}{目标：把不断放大的图像做成电影}
我们希望实现下面这个过程：

\begin{enumerate}
  \item 选定 Mandelbrot 集边界附近的一个点；
  \item 以这个点为中心生成一张 BMP 图像；
  \item 缩小观察窗口，再生成下一张 BMP 图像；
  \item 重复很多次，得到一系列逐步放大的帧；
  \item 最后把这些帧合成为一个视频文件。
\end{enumerate}

\vspace{0.5em}
也就是说，我们并不是一次性直接生成电影，而是先生成：

\[
\text{frame\_0000.bmp},\ \text{frame\_0001.bmp},\ \text{frame\_0002.bmp},\ \dots
\]
...
%------------------------------------------------
\begin{frame}{为什么这里适合使用 Shell 脚本}
Shell 脚本本身并不负责 Mandelbrot 集的数学计算。
...
\vspace{0.5em}
因此，这里 Shell 的角色不是“计算 Mandelbrot 集”，而是：

\[
\boxed{\text{负责自动化调度整个生成流程}}
\]
...
%------------------------------------------------
\begin{frame}[fragile]{主程序接口已经很适合批处理}
已知主程序接口为：

\begin{lstlisting}[language=bash]
./mandelbrot_app output.bmp center_x center_y half_width
\end{lstlisting}

\vspace{0.5em}
这表示每次运行程序时，只需要给出四个参数：

\begin{itemize}
  \item \texttt{output.bmp}：本次输出文件名；
  \item \texttt{center\_x}：观察中心的实部；
  \item \texttt{center\_y}：观察中心的虚部；
  \item \texttt{half\_width}：当前视野的半宽。
\end{itemize}
...
\begin{itemize}
  \item 固定 \texttt{center\_x}, \texttt{center\_y}；
  \item 每一帧都让 \texttt{half\_width} 再小一点。
\end{itemize}
\end{frame}

%------------------------------------------------
\begin{frame}[fragile]{\texttt{half\_width} 的几何含义}
如果当前观察中心为
\[
(center_x, center_y),
\]
并且水平半宽为 \texttt{half\_width}，那么图像对应的实部范围就是：
...
\begin{itemize}
  \item \texttt{half\_width} 越大，看到的区域越大；
  \item \texttt{half\_width} 越小，放大越深。
\end{itemize}
...
\[
half\_width \leftarrow half\_width \times zoom\_factor
\qquad (0 < zoom\_factor < 1)
\]
...
%------------------------------------------------
\begin{frame}[fragile]{脚本的基本思路}
整个脚本逻辑可以概括为：

\begin{enumerate}
  \item 先设定一个边界附近的中心点；
  \item 指定初始 \texttt{half\_width}；
  \item 设定总帧数 \texttt{FRAMES}；
  \item 在循环中反复调用 \texttt{mandelbrot\_app}；
  \item 每次生成一张新的 BMP；
  \item 每生成一帧，就把 \texttt{half\_width} 乘上一个缩放因子；
  \item 所有帧生成完成后，用 \texttt{ffmpeg} 合成视频。
\end{enumerate}
...
%------------------------------------------------
\begin{frame}[fragile]{Shell 脚本示例}
...
APP="./mandelbrot_app"  # 可执行程序名
OUTDIR="frames"         # 输出目录
# 放大中心
CENTER_X=-0.743643887037151
CENTER_Y=0.131825904205330

HALF_WIDTH=1.75         # 初始半宽
ZOOM_FACTOR=0.97        # 每帧放大比例
FRAMES=300              # 总帧数
...
%------------------------------------------------
%------------------------------------------------
\begin{frame}{脚本中的关键参数说明}
\begin{columns}[T,onlytextwidth]

\begin{column}{0.52\textwidth}
\begin{block}{主要参数}
\begin{itemize}
  \item \texttt{CENTER\_X}, \texttt{CENTER\_Y}：放大中心；
  \item \texttt{HALF\_WIDTH}：初始视野半宽；
  \item \texttt{ZOOM\_FACTOR}：每一帧的缩放比例；
  \item \texttt{FRAMES}：总帧数；
  \item \texttt{OUTDIR}：输出帧所在目录。
\end{itemize}
\end{block}
\end{column}
...
%------------------------------------------------
\begin{frame}[fragile]{为什么要用格式化文件名}
脚本中使用了：
...
\[
\texttt{frame\_0000.bmp},\ \texttt{frame\_0001.bmp},\ \dots
\]
...
\begin{enumerate}
  \item 文件名按字典序排列时，也能保持正确的时间顺序；
  \item \texttt{ffmpeg} 可以直接按这种命名规则读取全部帧。
\end{enumerate}
...
\[
\texttt{frame\_1.bmp},\ \texttt{frame\_10.bmp},\ \texttt{frame\_2.bmp}
\]
...
%------------------------------------------------
\begin{frame}[fragile]{边界探测点如何选择}
如果目标是观察 Mandelbrot 集边界的复杂结构，中心点应该选在边界附近，而不是明显在内部或外部。

\vspace{0.5em}
脚本中给出的经典点是：

\[
center_x = -0.743643887037151,\qquad
center_y = 0.131825904205330.
\]
...
\begin{itemize}
  \item 周围细节丰富；
  \item 连续放大后能看到很多层次；
  \item 适合生成“zoom movie”。
\end{itemize}

\vspace{0.5em}
也就是说，电影效果的好坏，不只取决于程序和脚本，也和所选中心点密切相关。
\end{frame}

%------------------------------------------------
\begin{frame}[fragile]{缩放因子如何影响视频效果}
脚本中用了：
...
这表示每一帧的视野半宽变成上一帧的 \(97\%\)。

\vspace{0.5em}
不同取值会产生不同效果：

\begin{itemize}
  \item \texttt{0.99}：放大较慢，更平滑；
  \item \texttt{0.97}：比较自然，适合演示；
  \item \texttt{0.95}：放大更快，节奏更强；
  \item \texttt{0.90}：变化很快，容易显得跳跃。
\end{itemize}
...
\[
\boxed{\text{电影的“镜头推进速度”}}
\]
\end{frame}

%------------------------------------------------
\begin{frame}[fragile]{生成 BMP 帧之后如何变成电影}
当所有 BMP 帧都生成完成后，可以用 \texttt{ffmpeg} 把它们合成为视频：

\begin{lstlisting}[language=bash]
ffmpeg -y -framerate 30 -i frames/frame_%04d.bmp \
...
\begin{itemize}
  \item \texttt{-framerate 30}：按每秒 30 帧播放；
  \item \texttt{-i frames/frame\_\%04d.bmp}：按编号顺序读取帧；
  \item \texttt{-c:v libx264}：使用 H.264 编码；
  \item \texttt{-pix\_fmt yuv420p}：使用兼容性较好的像素格式；
  \item 输出文件名为 \texttt{mandelbrot\_zoom.mp4}。
\end{itemize}
\end{frame}

%------------------------------------------------
\begin{frame}[fragile]{整个流程的结构图}
整个 Mandelbrot 放大电影的生成过程可以总结为：
...
\begin{itemize}
  \item C 程序：负责真正渲染 Mandelbrot 图像；
  \item Shell 脚本：负责自动化控制参数变化；
  \item \texttt{ffmpeg}：负责把静态帧拼成动态视频。
\end{itemize}
\end{frame}

%------------------------------------------------
\begin{frame}[fragile]{这种方法的优点}
用 shell 脚本批量生成放大电影有几个明显优点：

\begin{enumerate}
  \item \textbf{简单直接}：不需要改动 Mandelbrot 数学核心；
  \item \textbf{自动化程度高}：一次运行即可生成整个视频；
  \item \textbf{便于实验}：可以轻松更换中心点、缩放速度、帧数；
  \item \textbf{结构清晰}：计算、调度、视频编码三部分分离。
\end{enumerate}
...
\[
\text{C 负责数值与图像生成，Shell 负责自动化流程控制。}
\]
\end{frame}

%------------------------------------------------
\begin{frame}[fragile]{总结}
要生成 Mandelbrot 集边界放大的电影，并不需要程序本身直接输出视频。

\vspace{0.5em}
一种清晰而实用的方法是：

\begin{enumerate}
  \item 让已有 C 程序按命令行参数生成单张 BMP；
  \item 用 shell 脚本不断修改 \texttt{half\_width}，批量生成帧；
  \item 用 \texttt{ffmpeg} 把这些帧合成为电影。
\end{enumerate}
