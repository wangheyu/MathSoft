...
\begin{frame}{问题背景：为什么把 \texttt{bitmap.c} 做成库？}
\begin{itemize}
  \item 假设我们已经写好了 \texttt{bitmap.c} 和 \texttt{bitmap.h}，里面提供 BMP 图像读写函数。
  \item 如果多个程序都要调用这些函数，把它们做成\alert{库}会更方便复用。
  \item 常见两种库：
  \begin{itemize}
    \item \textbf{静态库（static library）}：文件通常是 \texttt{libbitmap.a}
    \item \textbf{动态库（shared library）}：文件通常是 \texttt{libbitmap.so}
  \end{itemize}
  \item 都可以让 \texttt{main.c} 通过
  \begin{center}
    \texttt{\#include "bitmap.h"}
  \end{center}
  来使用库中函数。
\end{itemize}
...
\textbf{典型目录：}
\begin{center}
\texttt{bitmap.c \quad bitmap.h \quad main.c}
\end{center}
\end{frame}

\begin{frame}[fragile]{把 \texttt{bitmap.c} 编译成静态库}
\textbf{步骤 1：先编译成目标文件}
\begin{lstlisting}[language=bash]
...
\textbf{结果：}
\begin{itemize}
  \item 生成文件 \texttt{libbitmap.a}
  \item 链接时，库中的目标代码会被复制进最终可执行文件
\end{itemize}

\textbf{使用示例：}
\begin{lstlisting}[language=bash]
gcc main.c ./libbitmap.a -o app_static
./app_static
\end{lstlisting}
\end{frame}

\begin{frame}[fragile]{把 \texttt{bitmap.c} 编译成动态库}
\textbf{步骤 1：生成位置无关代码（PIC）}
\begin{lstlisting}[language=bash]
gcc -fPIC -c bitmap.c -o bitmap_pic.o
\end{lstlisting}
...
\textbf{使用示例：}
\begin{lstlisting}[language=bash]
gcc main.c -L. -lbitmap -Wl,-rpath=. -o app_shared
./app_shared
\end{lstlisting}

\textbf{说明：}
\begin{itemize}
  \item \texttt{-fPIC}：让代码可被装载到任意内存位置
  \item \texttt{-shared}：生成共享库 \texttt{.so}
  \item 运行时程序需要能找到 \texttt{libbitmap.so}
\end{itemize}
\end{frame}

\begin{frame}{静态库与动态库的区别}
\small
\begin{center}
\begin{tabular}{p{2.6cm}p{4.2cm}p{4.2cm}}
\toprule
比较项 & 静态库 \texttt{libbitmap.a} & 动态库 \texttt{libbitmap.so} \\
\midrule
链接时机 & 编译/链接阶段把代码并入程序 & 运行时装载共享库 \\
可执行文件大小 & 较大 & 较小 \\
运行依赖 & 一般不依赖外部库文件 & 依赖系统能找到 \texttt{.so} \\
升级方式 & 重新链接应用程序 & 替换共享库即可（需兼容） \\
适合场景 & 部署简单、单文件发布 & 多程序共享、节省磁盘和内存 \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{一句话理解：}
\begin{itemize}
  \item 静态库：\alert{把库代码打包进程序}
  \item 动态库：\alert{程序运行时再去加载库}
\end{itemize}
\end{frame}


\begin{frame}[fragile]{完整命令总结}
\textbf{静态库：}
\begin{lstlisting}[language=bash]
gcc -c bitmap.c -o bitmap.o
ar rcs libbitmap.a bitmap.o
...
\begin{frame}[fragile]{同名动态库和静态库同时存在时}
如果目录中同时有：
\begin{center}
\texttt{libbitmap.a \quad 和 \quad libbitmap.so}
\end{center}

那么命令
\begin{lstlisting}[language=bash]
gcc main.c -L. -lbitmap -o app
\end{lstlisting}
通常会\alert{优先链接动态库 \texttt{libbitmap.so}}。

\vspace{0.5em}
\textbf{若要明确链接静态库，可直接写静态库文件名：}
\begin{lstlisting}[language=bash]
gcc main.c ./libbitmap.a -o app_static
\end{lstlisting}
...
也可以用链接器选项：
\begin{lstlisting}[language=bash]
gcc main.c -L. -Wl,-Bstatic -lbitmap \
    -Wl,-Bdynamic -o app_static
\end{lstlisting}
\end{frame}
...
\textbf{若要明确链接动态库，可写：}
\begin{lstlisting}[language=bash]
gcc main.c ./libbitmap.so -o app_shared
\end{lstlisting}
...
gcc main.c -L. -lbitmap -o app
\end{lstlisting}
并且当前目录下同时存在 \texttt{.so} 和 \texttt{.a}，
通常也会优先选择动态库。
...
若输出中出现 \texttt{libbitmap.so}，说明程序确实使用了动态库。
\end{frame}
