...
\subsection{表格、标题与浮动管理的示例}

在 LaTeX 中，表格通常放在 \texttt{table} 环境中，
而真正的数据区域一般写在 \texttt{tabular} 环境里。
下面的例子演示了表格制作、标题、标签和交叉引用的基本写法。
...
表 \ref{tab:values} 给出了 $x$、$x^2$ 和 $x^3$ 的一些取值。
这里 \texttt{tabular\{c c c\}} 中的三个 \texttt{c} 表示三列都采用居中对齐；
符号 \texttt{\&} 用来分隔列，
而 \texttt{\textbackslash\textbackslash} 用来结束一行。
横线 \texttt{\textbackslash hline} 可以用来分隔表头和数据。

与图片类似，表格所在的 \texttt{table} 环境也是浮动体。
因此，表格也不一定出现在源码当前位置的正下方，
而是由 LaTeX 根据页面布局自动安排。
在较长文档中，这种浮动管理同样有助于保持版面整齐。

通过 \texttt{\textbackslash caption} 和 \texttt{\textbackslash label}，
我们可以像引用图 \ref{fig:expfigure} 一样，
在正文中方便地引用表 \ref{tab:values}，
而不需要手工书写编号。
